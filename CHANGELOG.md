...
- **`Sprint`, `ClosedSprintRef`, and `SprintContext` ontology types** (`clearline/ontology/v1/core.py`) — preserve per-sprint id, name, start date, planned end date (`end_date` / Jira `endDate`), source completion boundary (`complete_date` / Jira `completeDate`), and fetch timestamp (`fetched_at`); `ClosedSprintRef` carries the full six-field set (previously dropped `start_date` and had no completion date); optional fields keep existing `start_date`/`end_date` consumers working unchanged; Jira adapter adds `jira_sprint_to_sprint` / `sprint_to_closed_ref` for normalization round-trip (IMUA-139). Carryover reconstruction utilities and metric logic remain out of scope.
